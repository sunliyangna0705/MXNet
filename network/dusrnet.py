...
class DuSRNet(nn.Module):
    def __init__(self, args):
        super(DuSRNet, self).__init__()
    ...
    def forward(self, X, Z, LI, Y, epoch):
       ...
       # Initial Z0
        ...
        for i in range(self.iter):
            ...
            ML.imwrite(mask[0,0,:,:].cpu().detach().numpy(), os.path.join(save_mask_dir, 'mask_temp_%i.png' %(i)))
            # mask_dir = '/data/sunlina/data/InitMask/anotherMask40.mat'
            # mask_data = loadmat(mask_dir)['mask']
            # mask = torch.from_numpy(mask_data).unsqueeze(0).unsqueeze(0).cuda()
            ...
            # value, idx = torch.sort(torch.abs(maskLine), descending=True)  # descending为alse，升序，为True，降序
            # idx1 = idx[0,:int(256*self.frc*256)]
            # idx0 = idx[0,int(256*self.frc*256):]
            # maskLine[0,idx1] = 1
            # maskLine[0,idx0] = 0
            # mask = maskLine.reshape(a1,b1,c1,d1)
